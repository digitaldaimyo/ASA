...
Note: Uses a simple character-level tokenizer, so the margin compares `P` vs `L`.
"""
...
def main() -> None:
    ...
    for prompt in PROMPTS:
        ...
        with torch.no_grad():
            logits, _ = model(input_ids, return_info=True)
        last_logits = logits[0, -1]
        paris_id = tokenizer.encode("P")[0]
        london_id = tokenizer.encode("L")[0]
