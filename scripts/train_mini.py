...
def batchify(tokens: List[int], seq_len: int, batch_size: int) -> torch.Tensor:
    chunks = len(tokens) // seq_len
...
def main() -> None:
    ...
    for step in range(1, args.steps + 1):
        batch = batchify(tokens, args.seq_len, batch_size=4).to(device)
